<?php
// views/pengembalian/tambah.php - Form Pengembalian
session_start();
...
$sql = "SELECT p.*, a.nama_lengkap as anggota 
        FROM peminjaman p
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "SELECT tanggal_jatuh_tempo FROM peminjaman WHERE peminjaman_id = :id";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(['id' => $peminjaman_id]);
    $pinjam = $stmt->fetch();

    $sql = "SELECT * FROM detail_peminjaman WHERE peminjaman_id = :id AND status_kembali = 'belum'";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(['id' => $peminjaman_id]);
    ...
    foreach ($details as $detail) {
        $denda = 0;
        if (date('Y-m-d') > $pinjam['tanggal_jatuh_tempo']) {
            $hari_terlambat = (strtotime(date('Y-m-d')) - strtotime($pinjam['tanggal_jatuh_tempo'])) / (60 * 60 * 24);
            $denda = $hari_terlambat * 2000;
        }

        $total_denda += $denda;

        $sql = "UPDATE detail_peminjaman 
                SET tanggal_kembali_aktual = CURDATE(), 
                    denda = :denda, 
                    status_kembali = 'sudah' 
                WHERE detail_id = :detail_id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(['denda' => $denda, 'detail_id' => $detail['detail_id']]);

        $sql = "UPDATE buku SET status = 'tersedia', stok = stok + 1 WHERE buku_id = :buku_id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(['buku_id' => $detail['buku_id']]);
    }

    $status = $total_denda > 0 ? 'terlambat' : 'dikembalikan';
    $sql = "UPDATE peminjaman SET status = :status WHERE peminjaman_id = :id";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(['status' => $status, 'id' => $peminjaman_id]);

    $sql = "INSERT INTO pengembalian (peminjaman_id, user_id, total_denda, catatan, status) 
            VALUES (:peminjaman_id, :user_id, :total_denda, :catatan, 'lengkap')";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([
...
<div class="sidebar">
    ...
    <ul class="menu">
        ...
        <li class="active"><span class="icon">↩️</span> <span>Pengembalian</span></li>
...
<div class="main-content">
    ...
    <div class="top-header">
        <div class="page-title">
            <h2>↩️ Form Pengembalian Buku</h2>
            <small>Proses pengembalian buku yang dipinjam</small>
        </div>
    </div>
    ...
    <div class="card-custom">
        <form method="POST">
            <div class="mb-3">
                ...
                <select name="peminjaman_id" class="form-control" required>
                    ...
                    <?php foreach ($peminjaman as $row): ?>
                        <option value="<?= $row['peminjaman_id'] ?>">
                            <?= htmlspecialchars($row['kode_transaksi']) ?> - <?= htmlspecialchars($row['anggota']) ?>
            ...
            <div class="alert alert-info">
                <strong>💡 Informasi:</strong> Denda akan dihitung otomatis sebesar Rp 2.000/hari jika melewati tanggal jatuh tempo.
            </div>
