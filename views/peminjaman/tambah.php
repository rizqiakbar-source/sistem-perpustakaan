<?php
// views/peminjaman/tambah.php - Form Peminjaman
session_start();
...
$anggota = $pdo->query("SELECT * FROM anggota WHERE status_aktif = 'aktif' ORDER BY nama_lengkap")->fetchAll();

// Ambil data buku yang tersedia
$buku = $pdo->query("SELECT * FROM buku WHERE status = 'tersedia' AND stok > 0 ORDER BY judul")->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($buku_ids)) {
        ...
        $sql = "INSERT INTO peminjaman (kode_transaksi, anggota_id, user_id, tanggal_jatuh_tempo, status) 
                VALUES (:kode, :anggota_id, :user_id, :tanggal_jatuh_tempo, 'dipinjam')";
...
<div class="sidebar">
    ...
    <ul class="menu">
        ...
        <li class="active"><span class="icon">📝</span> <span>Peminjaman</span></li>
...
<div class="main-content">
    ...
    <div class="top-header">
        <div class="page-title">
            ...
            <small>Silakan pilih anggota dan buku yang ingin dipinjam</small>
        </div>
    </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="anggota_id" class="form-control" required>
                        ...
                        <?php foreach ($anggota as $row): ?>
                            <option value="<?= $row['anggota_id'] ?>"><?= htmlspecialchars($row['nama_lengkap']) ?> (<?= htmlspecialchars($row['nis_nim']) ?>)</option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-6 mb-3">
                    ...
                    <?php if (count($buku) > 0): ?>
                        <?php foreach ($buku as $row): ?>
                            <div class="form-check">
                                <input type="checkbox" name="buku_ids[]" value="<?= $row['buku_id'] ?>" class="form-check-input" id="buku<?= $row['buku_id'] ?>">
                                <label class="form-check-label" for="buku<?= $row['buku_id'] ?>">
                                    <?= htmlspecialchars($row['judul']) ?> - <?= htmlspecialchars($row['penulis']) ?> (Stok: <?= $row['stok'] ?>)
                                </label>
                            </div>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <p class="text-danger">Tidak ada buku yang tersedia</p>
                    <?php endif; ?>
                </div>
            </div>
