<?php
// views/buku_anggota.php - Daftar Buku untuk Anggota
session_start();
...
// Ambil daftar wishlist anggota
$sql_wishlist = "SELECT buku_id FROM wishlist WHERE anggota_id = :anggota_id";
$stmt = $pdo->prepare($sql_wishlist);
...
<head>
    ...
    <style>
        ...
        .btn-wishlist-grid.active { color: #f9a825; }
        .btn-wishlist-grid.inactive { color: #ccc; }
        .btn-wishlist-grid:hover { transform: scale(1.2); }
...
<div class="main-content">
    ...
    <div class="top-header">
        <div class="page-title">
            <h2>📚 Daftar Buku</h2>
            <small>Koleksi buku perpustakaan</small>
        </div>
    </div>

    <div class="toolbar">
        <input type="text" placeholder="🔍 Cari Buku" id="searchInput" onkeyup="searchGrid()">
    </div>
    ...
    <div class="book-grid" id="bookGrid">
        <?php if (count($buku) > 0): ?>
            <?php foreach ($buku as $row): 
                ...
                $is_wishlist = in_array($row['buku_id'], $wishlist_ids);
                $sampul_path = "../upload/sampul/" . $row['buku_id'] . ".jpg";
            ?>
            <div class="book-card" data-nama="<?= strtolower($row['judul']) ?>" onclick="openDetail(<?= $row['buku_id'] ?>)">
                <div class="cover">
                ...
                <div class="info">
                    ...
                    <div class="status-stok">
                        ...
                        <div>
                            ...
                            <button class="btn-wishlist-grid <?= $is_wishlist ? 'active' : 'inactive' ?>" 
                                    onclick="event.stopPropagation(); toggleWishlist(<?= $row['buku_id'] ?>, this)">
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <div class="col-12 text-center">Belum ada data buku</div>
        <?php endif; ?>
    </div>
...
<script>
    ...
    function toggleWishlist(buku_id, button) {
        const isActive = button.classList.contains('active');
        fetch('wishlist_toggle.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: 'buku_id=' + buku_id + '&action=' + (isActive ? 'hapus' : 'tambah')
        })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
